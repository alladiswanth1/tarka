/**
 * Detect Claude Code / Codex CLIs on this machine and expose them as ordinary
 * Tarka providers (same list Solo, Debate, and Project already pick from).
 */
import { activeProviderId, providers, saveProviders, setActiveProviderId } from './providers.js';
import { $ } from './state.js';
import { setActiveProvider } from './ui/providers.js';

const LOCAL_PROVIDER_ID = { claude: 'local-claude', codex: 'local-codex' };
...
async function syncLocalAgentProviders() {
  ...
  for (const a of agents) {
    if (a.id !== 'claude' && a.id !== 'codex') continue;
...
function renderLocalAgentStrip(agents) {
  ...
  for (const a of agents) {
    ...
    li.className = 'local-agent-chip' + (a.ready ? ' ready' : a.installed ? ' installed' : '');
    const status = a.ready
      ? `signed in${a.version && a.version !== '?' ? ` · ${a.version}` : ''}`
      : a.installed
        ? 'installed — run `claude login` / `codex login`'
        : 'not installed';
