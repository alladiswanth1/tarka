...
/**
 * Serialize read-modify-write sequences per file. `writeJsonAtomic` makes each
 * WRITE atomic, not the sequence around it: the engine fires
 * `/api/projects/update { lastSeat }` per turn while the user may be saving a
 * rename, and A-read, B-read, A-write, B-write silently dropped the rename.
 * Two journal appends that both saw the rotation threshold both renamed, and
 * the second got ENOENT — losing events exactly when the journal was largest.
 */
...
function withFileLock(file, fn) {
  ...
  const tracked = run.finally(() => {
    if (fileLocks.get(file) === tracked) fileLocks.delete(file);
  });
  fileLocks.set(file, tracked);
  return run;
}
...
function appendJournal(project, events) {
...
module.exports = {
  withFileLock, writeJsonAtomic, readJson, readProjectsIndex, writeProjectsIndex,
