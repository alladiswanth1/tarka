...
const { readProjectsIndex, writeProjectsIndex, newProjectId, stateDirOf, loadProject,
        readJson, writeJsonAtomic, withFileLock, appendJournal, readJournalTail } = require('./state');
const { PROJECTS_INDEX } = require('../paths');
...
async function handleProjectRoute(req, res, url) {
  ...
  try {
    ...
    if (url.pathname === '/api/projects' && req.method === 'POST') {
      ...
      const created = await withFileLock(PROJECTS_INDEX, async () => {
        const idx = await readProjectsIndex();
        if (idx.some((p) => p.folder === folder)) return false;
      ...
      if (!created) return sendJsonRes(res, 409, { error: 'That folder is already assigned to a project' });
    ...
    if (url.pathname === '/api/projects/update' && req.method === 'POST') {
      ...
      const meta = await withFileLock(metaFile, async () => {
        const m = await readJson(metaFile, {});
        if (typeof patch.name === 'string' && patch.name.trim()) m.name = patch.name.trim().slice(0, 80);
        if (Array.isArray(patch.team)) m.team = patch.team.slice(0, 4);
        if (patch.settings && typeof patch.settings === 'object' && !Array.isArray(patch.settings)) {
          m.settings = { ...(m.settings || {}), ...patch.settings };
        }
        if (typeof patch.lastSeat === 'number') m.lastSeat = patch.lastSeat;
        m.updatedAt = Date.now();
        await writeJsonAtomic(metaFile, m);
        return m;
      });
      if (meta.name) {
    ...
    if (url.pathname === '/api/projects/delete' && req.method === 'POST') {
      ...
      const entry = await withFileLock(PROJECTS_INDEX, async () => {
        const idx = await readProjectsIndex();
        const found = idx.find((p) => p.id === id);
        if (found) await writeProjectsIndex(idx.filter((p) => p.id !== id));
        return found;
      });
    ...
    if (url.pathname === '/api/project/decision' && req.method === 'POST') {
      ...
      const decisions = await withFileLock(file, async () => {
      ...
      return sendJsonRes(res, 200, { ok: true, decisions });
    }
