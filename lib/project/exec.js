...
function projectExec(project, req, res, body) {
  return new Promise((resolve) => {
    ...
    const child = isWin
      ? spawn('cmd', ['/c', command], { cwd: project.folder, env: process.env })
      : spawn('/bin/sh', ['-c', command], { cwd: project.folder, env: process.env, detached: true });
    ...
    const settle = (extra) => {
      ...
      resolve({
        code: exitInfo == null || exitInfo.code == null ? -1 : exitInfo.code,
        signal: (exitInfo && exitInfo.signal) || undefined,
...
module.exports = { CATASTROPHIC_RE, projectExec };
