...
const CATASTROPHIC_RE = new RegExp(
  [
    ...
    // rm on a parent of the project folder: commands run with cwd = the
    // project, so `rm -rf ..` is a far likelier slip than `rm -rf /` and wipes
    // whatever the project lives in (~/projects).
    CMD_START + RM_FLAGS + String.raw`['"]?(?:\.\.\/?)+\*?['"]?` + TARGET_END,
...
function projectExec(project, req, res, body) {
  return new Promise((resolve) => {
    ...
    // stdin is closed, as in CI: with the default pipe left open, anything
    // that reads it (`cat`, `python3`, `read x`, a "continue? [y/N]" prompt)
    // sat for the full timeout and was reported as timedOut.
    const stdio = ['ignore', 'pipe', 'pipe'];
    const child = isWin
      ? spawn('cmd', ['/c', command], { cwd: project.folder, env, stdio })
      : spawn('/bin/sh', ['-c', command], { cwd: project.folder, env, detached: true, stdio });
