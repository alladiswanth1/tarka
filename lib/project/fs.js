...
async function listProjectTree(project, startRel, maxEntries = 600, maxDepth = 8) {
  ...
  const walk = async (dir, rel, depth) => {
    ...
    // Size every file in this directory at once. One awaited stat per file
    // made a 600-entry listing take ~85ms, and the tree is listed before
    // every Project turn and again after it (the inspector) — all of it
    // latency in front of the model call. Capped at what can still be listed.
    const files = items.filter((it) => it.isFile()).slice(0, Math.max(0, maxEntries - out.length));
    const sizes = new Map(
      await Promise.all(
        files.map(async (it) => {
          try {
            return [it.name, (await fsp.stat(path.join(dir, it.name))).size];
          } catch {
            return [it.name, 0];
          }
        })
    ...
    for (const it of items) {
      ...
      if (it.isDirectory()) {
        ...
        out.push({ path: childRel, size: sizes.get(it.name) || 0 });
      }
    }
  };
