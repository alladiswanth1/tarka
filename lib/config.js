...
// Behind a reverse proxy on the same machine EVERY client arrives from
// 127.0.0.1, so the loopback-only routes (Project Mode file writes and shell,
// local CLI agents) and the SSRF containment would treat the internet as the
// local user. Set this to say "nobody is local". Not inferred from
// TARKA_ALLOWED_HOSTS: a LAN user listing a hostname is not behind a proxy.
const BEHIND_PROXY = /^(1|true|yes)$/i.test(String(process.env.TARKA_BEHIND_PROXY || '').trim());
...
module.exports = {
  PORT, HOST, UPSTREAM_TIMEOUT_MS, MODELS_TIMEOUT_MS, SSE_KEEPALIVE_MS, REFERER_HOST, BEHIND_PROXY,
