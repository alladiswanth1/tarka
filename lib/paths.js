...
// TARKA_DATA_DIR relocates the project index (tests point it at a temp dir so
// a suite run never adds "Routes (folder missing)" to the user's real list).
const DATA_DIR = (() => {
  const raw = String(process.env.TARKA_DATA_DIR || '').trim();
  return raw && path.isAbsolute(raw) ? path.resolve(raw) : path.join(APP_DIR, 'data');
})();
