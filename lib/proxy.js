...
const { parseBody, writeSse, createSseCoalescer } = require('./http');
...
async function handleChat(req, res) {
  ...
  const startAttempt = (state) => {
    ...
    upstream = lib.request(options, (upRes) => {
       ...
       */
      let dataLines = [];
      /** Token deltas from one upstream chunk leave as one event per type run. */
      ...
      const handleUpstreamEvent = (parsed) => {
        ...
        if (parsed.error) {
          ...
          out.write({ type: 'error', error: msg });
          return;
        }
      ...
      const handleUpstreamLine = (line) => {
        ...
        // its last line arrives, which is exactly the framing we want. The
        // parse that proves it complete IS the event: parsing it a second time
        // in flushEvent doubled the proxy's per-token JSON work.
        const joined = dataLines.length === 1 ? dataLines[0] : dataLines.join('\n').trim();
        if (joined === '[DONE]') {
          dataLines = [];
          return;
        }
      ...
      upRes.on('error', (err) => {
        out.write({ type: 'error', error: err.message });
        if (!res.writableEnded) res.end();
        cleanup();
      });
    });
