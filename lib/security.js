...
const os = require('os');
const { EXTRA_ALLOWED_HOSTS, BEHIND_PROXY } = require('./config');
...
/**
 * Is this one of THIS machine's own interface addresses? A public IP on the
 * host is not "private", so a remote client could point the Base URL at
 * Tarka's own public address: that request arrives from 127.0.0.1 and would
 * pass every loopback-only check. Every project route is exact-match today, so
 * it is not exploitable yet — it is one prefix-matched route away from being a
 * loopback-laundering primitive.
 */
function isOwnAddress(ip) {
  const want = String(ip || '').toLowerCase().replace(/^::ffff:/, '');
  ...
  for (const list of Object.values(ifaces)) {
    for (const entry of list || []) {
      const addr = String(entry.address || '').toLowerCase().split('%')[0];
 ...
 * if any address is private (prevents public-hostname → private-IP bypass) or
...
async function assertProxyTargetAllowed(req, targetUrl) {
  ...
  if (isIpLiteral(hostname)) {
    if (isOwnAddress(hostname)) {
      const err = new Error('Refusing to proxy to this server’s own address from a non-loopback client');
      err.statusCode = 403;
      throw err;
    }
  ...
  for (const r of results) {
    if (isPrivateIp(r.address) || isOwnAddress(r.address)) {
      const err = new Error(
        'Refusing to proxy to internal/private host from a non-loopback client'
      );
      err.statusCode = 403;
...
module.exports = {
  isLoopbackRemote, isIpLiteral, isPrivateIp, isBlockedInternalHostname, hostnameFromHeader, isTrustedHostHeader, isSameOriginRequest, isCsrfSafeContentType, assertProxyTargetAllowed, applyPinnedLookup, assertLoopback, isOwnAddress
};
